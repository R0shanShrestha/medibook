...
const AppointmentCard = ({ appointment }) => {
  ...
  const { cancelAppointment, completeAppointment } = useContext(
    ...
    >
        ...
        <div className="min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3 text-xs sm:text-sm text-slate-600 mt-1 sm:mt-2">
            <span className="flex items-center gap-1">
              <Calendar size={14} /> {slotDateFormat(appointment?.slotDate)}
            </span>
            <span className="flex items-center gap-1">
              <Clock size={14} /> {appointment?.slotTime}
            </span>
            <span className="flex items-center gap-1">
              <Wallet size={14} /> Rs. {appointment?.docId?.fee || 0}
            </span>
            {/* <span
      ...
      {appointment?.cancelled ? (
        ...
        <div className="flex flex-col sm:flex-row gap-2 justify-end w-full md:w-auto">
          <button
            onClick={() => {
              cancelAppointment(appointment?._id, setLoading);
            }}
          ...
          <button
            onClick={() => {
              completeAppointment(appointment?._id, setLoading);
            }}
